...
class RocketCodegen(BaseCodegen):
    ...
    def _emit_execute_control_body(self):
        ...
        L += [f"{ind}// Tracking error e = s - s_ref, with a heading wrap on the yaw error.",
              f"{ind}// NOTE: unlike the quadrotor, NO yaw-frame rotation of the horizontal",
              f"{ind}// errors is applied here — and none is needed. In Rm = Ry(a)*Rx(b)*Rz(psi)",
              f"{ind}// the roll Rz is the innermost rotation and leaves body-z invariant, so",
              f"{ind}// the thrust direction (and the isotropic lateral drag) do not depend on",
              f"{ind}// psi: the plant is roll-invariant and the yaw=0 gain is valid at any psi.",
              f"{ind}// Rotating the errors here would MIS-align the loop by psi (unstable",
              f"{ind}// beyond ~0.8 rad; verified by closed-loop eigenvalues).",
